...
Exit code
---------
0  -> no error-level findings (and, with ``--strict``, no warnings either)
1  -> at least one finding that fails the run (problem locations are printed)
2  -> usage / file error
"""
...
# severity: "error" fails the run; "warn" is reported and fails only under --strict.
...
CHECKS = [
    ...
    ("external <iframe src>", "error",
     ...
     "an iframe must not load a remote document"),
...
def validate_file(path: Path) -> tuple[int, int]:
    """Validate one file. Return (error_count, warn_count); (-1, 0) if unreadable."""
    if not path.is_file():
    ...
    if has_iframe and not has_sandbox:
        warns += 1
        print('  [warn ] iframe present without a sandbox attribute — this bundle needs no '
              'iframe at all; if you add one, give it sandbox="allow-scripts"')
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        ...
        description=(
            "Check the generated lesson bundle for unsafe or non-self-contained patterns "
